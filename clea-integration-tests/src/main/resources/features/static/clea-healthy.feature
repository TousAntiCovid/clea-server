Feature: Several healthy visitors visit different places
  ...
  Background:
    ...
    Given VType of "restaurant", VCategory1 of "fastfood" and VCategory2 of 1 has risk configuration of (Threshold , ExposureTime, Risklevel) for backward (3,30,3.0) and for forward (1,30,2.0)
    ...
    Given "Chez McDonald's" created a static QRCode at 11:00, 13 days ago with VType as "restaurant" and with VCategory1 as "fastfood" and with VCategory2 as 1 and with a periodDuration of "24 hours"
  ...
  Scenario: Overlaps - 3 days with 3 different location and malformed pivot date for 1 person
   ...
   When "Yäel" declares himself sick with malformed pivot date
   When Cluster detection triggered
   When "Julie" asks for exposure status
   When "Mahe" asks for exposure status
  ...
  Scenario: Nominal case
    ...
    Then Exposure status should reports "Julie" as not being at risk
     ...
     Given "Yäel" recorded a visit to "Chez McDonald's" at 13:45, 4 days ago
     Given "Julie" recorded a visit to "Chez McDonald's" at 13:40, 4 days ago
     When "Yäel" declares himself sick with malformed QrCode
     When Cluster detection triggered
     Then "Yäel" cannot send his visits
     Then Exposure status should reports "Julie" as not being at risk
     ...
     When "Yäel" declares himself sick with a 5 days ago pivot date with no QRCode
     When Cluster detection triggered
     Then "Yäel" cannot send his visits
     Then Exposure status should reports "Julie" as not being at risk
     ...
     Given "Yäel" recorded a visit to "Chez McDonald's" at 13:45, 4 days ago
     Given "Julie" recorded a visit to "Chez McDonald's" at 13:40, 4 days ago
     When "Yäel" declares himself sick with malformed scan time
     When Cluster detection triggered
     Then "Yäel" has 1 rejected visit
     Then Exposure status should reports "Julie" as not being at risk
...
#     Given "Julie" recorded a visit to "Chez McDonald's" at 13:45, 4 days ago
#     When "Yäel" declares himself sick with malformed token
#     When Cluster detection triggered
#     Then "Yäel" cannot send his visits
#     Then Exposure status should reports "Julie" as not being at risk